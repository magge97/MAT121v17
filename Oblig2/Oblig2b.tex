...
\usepackage{graphicx} % For ï¿½ inkludere grafikk
...
\end{align*}
Den augmenterte matrisen $\begin{bmatrix} A & \bold{b} \end{bmatrix}$ er inkonsistent og følgelig er $\bold{b} \notin \text{Col} A$.

% 2)
\item[2)]
Eksempel på  $4 \times 3$ matrise med rank($A$) $ = 1 $:
\begin{align*}
...
\includegraphics[width=10cm, height=6cm]{oppg6}
...
\pagebreak
\item[8)]
Utfører rekkeoperasjoner på matrisen
\begin{align*}
...
Da følger det per definisjon av inverterbarhet at $(I-A)$ er inverterbar med $(I-A)^{-1} = I + A + A^2 + ... + A^{p-1}$.
\end{document}
